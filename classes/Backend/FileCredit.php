...
class FileCredit extends \Backend implements \executable
{
	...
	public static function findFileCreditPages($pid = 0, $domain = '', $blnIsSitemap = false, $strLanguage = '')
	{
		...
		while ($objPages->next()) {
				...
				if ((!$objPages->protected || \Config::get('indexProtected'))) {
					...
					if ($objPages->published && ($objPages->start == '' || $objPages->start <= $time) && ($objPages->stop == '' || $objPages->stop > ($time + 60))) {
						...
						$objArticle = $objDatabase->prepare(
							"SELECT * FROM tl_article WHERE pid=? AND (start='' OR start<='$time') AND (stop='' OR stop>'" . ($time + 60) . "') AND published='1' AND showTeaser='1' ORDER BY sorting"
						)
							->execute($objPages->id);

						while ($objArticle->next()) {
							$arrPages[] = $domain . static::generateFrontendUrl(
									$objPages->row(),
									'/articles/' . (($objArticle->alias != '' && !\Config::get('disableAlias')) ? $objArticle->alias : $objArticle->id),
									$strLanguage
								);
						}
					}
			...
			// Get subpages
			if ((!$objPages->protected || \Config::get('indexProtected')) && ($arrSubpages = static::findFileCreditPages($objPages->id, $domain, $blnIsSitemap, $strLanguage)) != false) {
