		...
		foreach ($this->arrItems as $objItem)
		{
			...
			if (is_array($objItem->enclosure))
			{
				foreach ($objItem->enclosure as $arrEnclosure)
				{
					...
					// Show file copyrights
					$objResult = \Database::getInstance()->prepare('SELECT * from tl_files WHERE path = ? AND copyright != ""')->limit(1)->execute(str_replace(\Environment::get('base'), '', $arrEnclosure['url']));
